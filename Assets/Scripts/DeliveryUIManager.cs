...
public class DeliveryUIManager : MonoBehaviour
{
    ...
    void Start()
    {
        if(driver != null)
        {
            ...
            driver.driverEvents.OnDeliveryCompleted.AddListener(OnDeliveryComplted);
        }
    ...
    void ShowMessage(string message, Color color)                   // 메세지 표시 함수
    {
        if(messageText != null)
        {
            ...
            StartCoroutine(ClaerMessageAgterDelay(2f));
        }
    }

    IEnumerator ClaerMessageAgterDelay(float delay)                 // 일정 시간 지난 후 사라진다.
    {
    ...
    private void OnDestroy()                // 객체가 파괴 될 때 시점
    {
        if (driver != null)
        {
            ...
            driver.driverEvents.OnDeliveryCompleted.RemoveListener(OnDeliveryComplted);
        }
    }
}
