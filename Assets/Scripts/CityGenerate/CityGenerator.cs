...
public class CityGenerator : MonoBehaviour
{
    ...
    void GenerateCity()
    {
        CreateGround();   //땅 생성
        CreateRoads();     //길 생성 
        CreateBuildings();  //건물 생성
        CreateStreetLights();   //가로등 배치
     }
    ...
    bool IsRoad(int x, int z)                   //길 배치 판단 함수
    {
    ...
    void CreatePark(int x, int z)
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            Vector3 treePos = new Vector3(x * blockSize + Random.Range(-blockSize * 0.3f, blockSize * 0.3f), 2f,
    ...
    Color GetBuildingColor(int x, int z)          //건물 색 설정
    {
