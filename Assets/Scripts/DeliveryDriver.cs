...
public class DeliveryDriver : MonoBehaviour
{
    ...
    {
        // 초기 상태 Event 발생
        driverEvents.OnMoneyChanged?.Invoke(currnetMoney);
    ...
    void HandleMovement()
    {
        ...
        float vercital = Input.GetAxis("Vertical");

        Vector3 moveDirection = new Vector3(horizontal, 0, vercital);
    ...
    {
        currnetMoney += amount;
        driverEvents.OnMoneyChanged?.Invoke(currnetMoney);      // 돈 획득 후 이벤트 처리
    }

    public void CompleteDelivery()                              // 배당 완료 함수
    {
        deliveryCount++;
    ...
    {
        return $"돈 {currnetMoney:F0} 원 | 배터리 : {batteryLevel:F1}% | 배달 : {deliveryCount} 건";
    }
